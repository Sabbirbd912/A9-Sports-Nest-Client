...
const editBooking = ({ params }) => {
  ...
  const totalPrice =
    Number(hours || 0) * Number(booking?.facility?.price_per_hour || 0);
  ...
  const inputStyle =
    "w-full px-4 py-3 border border-gray-200 rounded-lg text-black placeholder:text-black placeholder:opacity-50 focus:outline-none focus:ring-2 focus:ring-[#97D700] focus:border-[#97D700]";
  ...
  const handleEditBooking = async (e) => {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/booking/${booking._id}/update`,
      {
        method: "PATCH",
        headers: {
        ...
        <form onSubmit={handleEditBooking} className="space-y-5">
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div>
              ...
              <select name="time_slot" className={inputStyle} defaultValue="">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-4 mt-5">
            ...
            <Link
              href={`/bookings`}
              className="bg-[#d77300] hover:bg-red-500 text-[#002D3A] text-center flex justify-center items-center font-bold px-8 py-3 rounded-full transition"
            >
              Cancel
            </Link>
          </div>
